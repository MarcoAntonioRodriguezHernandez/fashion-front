...
class RefreshProductMedia implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        $response = $this->refreshProductService->refreshProductMediaMarketplace($this->productSlug, $this->colorShadeId);

        if ($response->failed()) {
            throw new \Exception('Failed to refresh product media [' . $this->productSlug . ']: ' . $response->getBody());
        }
    }
