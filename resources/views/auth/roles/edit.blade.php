<x-layouts.master-layout title="Editar rol" :cardTitle="$data->name">
    <form class="form" id="editRole" action="{{ route('auth.roles.edit') }}" method="POST">
        @csrf
        @method('PUT')
                    ...
                    <div class="col mx-5">
                        ...
                        <div class="row mb-6">
                            ...
                            <div class="fv-row col-lg-8">
                                <x-layouts.inputs typeInput="justInput" typeofInput="text" name="name" id="name" placeholder="Nombre del Rol" value="{{ $data->name }}" />
                            </div>
                            <!--end::Col-->
                        </div>
                        <!--end::Input group-->
                        ...
                        <div class="row mb-6">
                            ...
                            <!--begin::Col-->
                            <div class="fv-row col-lg-8">
                                <x-layouts.inputs typeInput="justInput" typeofInput="text" name="description" id="description" placeholder="Descripción" value="{{ $data->description }}" />
                            </div>
                            <!--end::Col-->
                        </div>
                        <!--end::Input group-->
                                                ...
                                                @foreach (PermissionTypes::getAllNames() as $value => $name)
                                                    ...
                                                    <label class="form-check form-check-sm form-check-custom form-check-solid me-5 me-lg-20">
                                                        <input class="form-check-input" type="checkbox" value="1" name="permissions[{{ $module->id }}][{{ $value }}]"
                                                            @checked(old('permissions.' . $module->id . '.' . $value, $data->permissions[$module->id][$value] ?? false))
        ...
        <x-btn-cancel-save routeCancel="{{ route('auth.roles.index') }}" 
