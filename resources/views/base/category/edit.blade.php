<x-layouts.master-layout title="Editar categoría" cardTitle="Editar {{ $data->name }}">

    <div class="form_padding">
        <!--begin::Form-->
        <form id="staffAdd" class="form d-flex flex-column flex-lg-row" action="{{ route('base.category.edit') }}"
            ...
            <div class="d-flex flex-column flex-row-fluid gap-7 gap-lg-10">
                ...
                <div class="d-flex flex-column gap-7 gap-lg-10">
                    ...
                    <div class="card card-flush py-4">
                        ...
                        <div class="card-header">
                            ...
                            <div class="card-body pt-0">
                                ...
                                <!--begin::Input group-->
                                <div class="mb-10 fv-row">
                                    <label class="form-label">Categoría Padre <span class="ms-1" data-bs-toggle="tooltip" title="La categoría padre es la categoría superior en una estructura jerárquica. Se utiliza para organizar las categorías en niveles. Si esta categoría tiene una categoría padre, será una subcategoría de esa categoría.">
                                    ...
                                    <div class="d-flex gap-3">
                                        <select name="parent_category_id" id="parent_category_id"
                                            ...
                                            @foreach ($categories as $parentCategory)
                                                <option value="{{ $parentCategory->id }}" @selected($parentCategory->id == old('parent_category_id', $data->parent_category_id))>
                                                    {{ $parentCategory->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="d-flex gap-3">
                                        <select name="characteristics[]" id="characteristics" class="form-select form-control" data-control="select2" data-hide-search="true" data-placeholder="Habilitar todas" multiple>
                                            @foreach ($characteristics as $characteristic)
                                                <option value="{{ $characteristic->id }}" @selected(in_array($characteristic->id, old('characteristics', $data->characteristics()->pluck('characteristics.id')->toArray())))>
                                                    {{ $characteristic->name }}
                                                </option>
                                            @endforeach
                                        </select>
                    ...
                    <!--end::Tab content-->
                    <x-btn-cancel-save
                    routeCancel="{{ route('base.category.index') }}"
                    :module="ModuleAliases::CATEGORY"
                    />
                </div>
                <!--end::Main column-->
        </form>
