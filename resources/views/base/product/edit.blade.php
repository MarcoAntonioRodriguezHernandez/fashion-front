<x-layouts.master-layout title="Editar producto {{ $data->name }}" cardTitle="Editar producto {{ $data->name }}">
    <style>
        .color-info {
            display: flex;
            align-items: center;
    ...
    <div class="form_padding">
        <form id="staffAdd" action="{{ route('base.product.edit') }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                        ...
                        <div class="card-header">
                            ...
                            <div class="card-body pt-0 mt-5 row">
                                ...
                                <div class="mb-10 col-md-6 fv-row">
                                    ...
                                    <!--end::Label-->
                                    <!--begin::Input-->
                                    <x-layouts.inputs typeInput="justInput" name="title" id="title" placeholder="Título" value="{{ old('title', $data->title) }}" />
                                    <!--end::Input-->
                                </div>
                                <!--end::Input group-->
                                    ...
                                    <div class="d-flex gap-3">
                                        <select name="designer_id" id="designer_id" class="form-select form-control" data-control="select2" data-hide-search="false" data-placeholder="">
                                            ...
                                            @foreach ($designers as $designer)
                                                <option value="{{ $designer->id }}" @selected($designer->id == old('designer_id', $data->designer_id))>{{ $designer->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <!--end::Input-->
                                ...
                                <div class="mb-10 fv-row">
                                    ...
                                    <!--end::Label-->
                                    <!--begin::Input-->
                                    <textarea class="form-control" placeholder="Descripción" name="description" id="description" cols="20" rows="5">{{ old('description', $data->description) }}</textarea>
                                    <!--end::Input-->
                                </div>
                                <!--end::Input group-->
                                ...
                                <div class="mb-10 col-md-6 fv-row">
                                    ...
                                    <!--begin::Input-->
                                    <div class="d-flex gap-3">
                                        <x-layouts.inputs typeInput="justInput" typeofInput="text" name="origin_code" id="origin_code" placeholder="Código de Origen" value="{{ old('origin_code', $data->origin_code) }}" />
                                    </div>
                                    <!--end::Input-->
                                </div>
                                <!--end::Input group-->
                                ...
                                <div class="mb-10 col-md-6 fv-row">
                                    ...
                                    <!--begin::Input-->
                                    <div class="d-flex gap-3">
                                        <x-layouts.inputs typeInput="justInput" typeofInput="text" name="internal_code" id="internal_code" placeholder="Código CM" value="{{ old('internal_code', $data->internal_code) }}" />
                                    </div>
                                    <!--end::Input-->
                                </div>
                                <!--end::Input group-->
                                ...
                                <div class="mb-10 col-md-6 fv-row">
                                    ...
                                    <!--end::Label-->
                                    <div class="d-flex gap-3">
                                        <x-layouts.inputs typeInput="justInput" typeofInput="number" name="full_price" id="full_price" placeholder="Precio venta sugerido" value="{{ old('full_price', $data->full_price) }}" />
                                    </div>
                                </div>
                                <!--end::Input group-->
                                ...
                                <div class="mb-10 col-md-6 fv-row">
                                    ...
                                    <select class="form-select form-control" name="pricing_scheme_id" id="pricing_scheme_id" data-control="select2" data-hide-search="true" data-placeholder="">
                                        ...
                                        @foreach ($pricingSchemes as $pricingScheme)
                                            <option value="{{ $pricingScheme->id }}" @selected($pricingScheme->id == old('pricing_scheme_id', $data->pricing_scheme_id))>{{ $pricingScheme->sku_4->price . ' - ' . $pricingScheme->msrp }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <!--end::Input group-->
                                    ...
                                    <div class="d-flex gap-3">
                                        <select name="sale_type" id="sale_type" class="form-select form-control" data-control="select2" data-hide-search="true" data-placeholder="">
                                            ...
                                            @foreach (ProductSaleTypes::getAllNames() as $value => $name)
                                                <option value="{{ $value }}" @selected($value == old('sale_type', $data->sale_type))>{{ $name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <!--end::Input-->
                                    ...
                                    <div class="d-flex gap-3">
                                        <select name="category_id" id="category_id" class="form-select form-control" data-control="select2" data-hide-search="false" data-placeholder="">
                                            ...
                                            @foreach ($categories as $category)
                                                <option value="{{ $category->id }}" @selected($category->id == old('category_id', $data->category_id))>{{ $category->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <!--end::Input-->
                                ...
                                <div class="mb-10 col-md-6 fv-row">
                                    ...
                                    <div class="d-flex gap-3">
                                        <select name="desired" id="desired" class="form-select form-control" data-control="select2" data-hide-search="true" data-placeholder="">
                                            ...
                                            <option value="1" @selected('1' == old('desired', $data->desired))>Deseado</option>
                                            <option value="0" @selected('0' == old('desired', $data->desired))>No Deseado</option>
                                        </select>
                                    </div>
                                    <!--end::Input-->
                                </div>
                                ...
                                <div class="mb-10 fv-row">
                                    ...
                                    <x-layouts.inputs typeInput="justInput" name="sku" id="sku" placeholder="Sku" value="{{ old('sku', $data->sku) }}" />
                                    <!--end::Input-->
                                </div>
                                <!--end::Input group-->
                                ...
                                <div class="mb-10 fv-row">
                                    ...
                                    <div class="d-flex gap-3">
                                        <select class="form-control select2" data-control="select2" data-tags="true" multiple name="tags[]" id="tags">
                                            ...
                                            @foreach ($tags as $tag)
                                                <option value="{{ $tag->id }}" @selected(in_array($tag->id, old('tags', $data->tags()->pluck('tags.id')->toArray())))>{{ $tag->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <!--end::Input-->
                                    ...
                                    <div class="d-flex gap-3">
                                        <select class="form-control select2" data-control="select2" multiple name="providers[]" id="providers">
                                            ...
                                            @foreach ($providers as $provider)
                                                <option value="{{ $provider->id }}" @selected(in_array($provider->id, old('providers', $data->providers()->pluck('providers.id')->toArray())))>{{ $provider->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <!--end::Input-->
                    ...
                    <div class="card card-flush py-4">
                        ...
                        <div class="card-header">
                            ...
                            <div class="card-body pt-0 mt-5">
                                <x-product.characteristic-selection :characteristics="$characteristics" :selected="$data->characteristics->pluck('id')->toArray()" />
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <!--begin::Image column-->
            <x-product.image-product :colors="$colors" :frontImages="$data->frontImages" :backImages="$data->backImages" :rightImages="$data->rightImages" :leftImages="$data->leftImages" />
            <!--end::Image column-->

            <x-btn-cancel-save routeCancel="{{ route('base.product.index') }}" :module="ModuleAliases::PRODUCT">
                <!--begin::Button-->
                <a href="{{ route('base.product.variants.view', $data->id) }}" class="btn btn-success me-5">Ver variantes</a>
                <!--end::Button-->
                <!--begin::Button-->
                <a href="{{ route('base.supply.create.view', $data->id) }}" class="btn btn-info me-5">Ver artículos</a>
                <!--end::Button-->
            </x-btn-cancel-save>
        </form>
    </div>
